...
#pragma pack(pop)
	...
	void playBackgroundMusic()
	{
		...
		if (!_channels[enumValue(ChannelId::bgm)].isPlaying())
		{
			...
			{
				_channels[enumValue(ChannelId::bgm)].setVolume(Config::getNew().audio.bgmVolume);
	...
	void playTitleScreenMusic()
	{
		...
		if (isTitleMode() && _audioInitialised && _audioIsEnabled && Config::getNew().audio.playTitleMusic)
		{
			if (!channel->isPlaying())
			{
				...
				{
					channel->setVolume(Config::getNew().audio.titleVolume);
					channel->play(true);
				}
			}
		}
	...
	// previously ^^^
	void setChannelVolume(ChannelId channelId, int32_t volume)
	{
		auto& cfg = Config::getNew();

		std::unordered_map<ChannelId, int32_t*> channelIdToConfig = {
			{ ChannelId::bgm, &cfg.audio.bgmVolume },
			{ ChannelId::ambient, &cfg.audio.ambientVolume },
			{ ChannelId::title, &cfg.audio.titleVolume },
			{ ChannelId::vehicle_0, &cfg.audio.vehicleVolume },
		};

		if (*channelIdToConfig.at(channelId) == volume)
		{
			return;
		}

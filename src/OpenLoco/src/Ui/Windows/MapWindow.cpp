...
namespace OpenLoco::Ui::Windows::MapWindow
{
    ...
    static constexpr int16_t kRenderedMapWidth = TileManager::kMapColumns * 2;
    ...
    static std::array<Point, 4> kViewFrameOffsetsByRotation = { {
        { TileManager::kMapColumns - 8, 0 },
        { kRenderedMapWidth - 8, TileManager::kMapRows },
        { TileManager::kMapColumns - 8, kRenderedMapHeight },
        { -8, TileManager::kMapRows },
    } };
    ...
    {
        pos.x = ((pos.x + 8) - TileManager::kMapColumns) / 2;
        pos.y = ((pos.y + 8)) / 2;
        ...
        switch (getCurrentRotation())
        {
            case 0:
                return location;
            case 1:
                return { static_cast<coord_t>(TileManager::kMapWidth - 1 - location.y), location.x };
            case 2:
                return { static_cast<coord_t>(TileManager::kMapWidth - 1 - location.x), static_cast<coord_t>(TileManager::kMapHeight - 1 - location.y) };
            case 3:
                return { location.y, static_cast<coord_t>(TileManager::kMapHeight - 1 - location.x) };
        }
    ...
    static Point locationToMapWindowPos(Pos2 pos)
    {
        ...
        switch (getCurrentRotation())
        {
            case 3:
                std::swap(x, y);
                x = TileManager::kMapWidth - 1 - x;
                break;
            case 2:
                x = TileManager::kMapWidth - 1 - x;
                y = TileManager::kMapHeight - 1 - y;
                break;
            case 1:
                std::swap(x, y);
                y = TileManager::kMapHeight - 1 - y;
        ...
        return Point(-x + y + TileManager::kMapColumns - 8, x + y - 8);
    }
    ...
    {
        for (auto rowCountLeft = TileManager::kMapColumns; rowCountLeft > 0; rowCountLeft--)
        {
            // Coords shouldn't be at map edge
            if (!(pos.x > 0 && pos.y > 0 && pos.x < TileManager::kMapWidth - kTileSize && pos.y < TileManager::kMapHeight - kTileSize))
            {
                pos += delta;
        ...
        _drawMapRowIndex++;
        if (_drawMapRowIndex > TileManager::kMapColumns)
        {
            _drawMapRowIndex = 0;
        }
    }
    ...
    static void setMapPixels(const Window& self)
    {
        ...
        auto offset = _drawMapRowIndex * (kRenderedMapWidth - 1) + (TileManager::kMapRows - 1);
        ...
        switch (WindowManager::getCurrentRotation())
        {
            case 0:
                ...
                break;
            case 1:
                pos = Pos2(TileManager::kMapWidth - kTileSize, _drawMapRowIndex * kTileSize);
                delta = { -kTileSize, 0 };
                break;
            case 2:
                pos = Pos2((TileManager::kMapColumns - 1 - _drawMapRowIndex) * kTileSize, TileManager::kMapWidth - kTileSize);
                delta = { 0, -kTileSize };
                break;
            case 3:
                pos = Pos2(0, (TileManager::kMapColumns - 1 - _drawMapRowIndex) * kTileSize);
                delta = { kTileSize, 0 };
                break;
        }
    ...
    static void drawScroll(Window& self, Gfx::DrawingContext& drawingCtx, [[maybe_unused]] const uint32_t scrollIndex)
    {
        ...
        Gfx::getG1Element(0)->width = TileManager::kMapColumns * 2;
        Gfx::getG1Element(0)->height = TileManager::kMapRows * 2;
